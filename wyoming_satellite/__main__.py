...
from .utils import AudioBuffer, chunk_samples, multiply_volume, run_event_command
...
class SatelliteEventHandler(AsyncEventHandler):
    ...
    #                         # Cancel current pipeline with audio stop
    #                         chunk = AudioChunk.from_event(mic_event)
    ...
    #     try:
    #         mic_client = self._make_mic_client()
    #         wake_client = await self._connect_to_wake()
    ...
    # async def _connect_to_wake(self) -> AsyncClient:
    ...
    #             return AsyncClient.from_uri(self.cli_args.wake_uri)
    #         except (ConnectionError, OSError):
    #             _LOGGER.exception(
    #                 "Unexpected error connecting to wake service. "
    #                 "Reconnecting in %s second(s)",
    #                 _WAKE_RECONNECT_SECONDS,
    #             )
    #             await asyncio.sleep(_WAKE_RECONNECT_SECONDS)
